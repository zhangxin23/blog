...
task :post do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "tags: #{tags}"
    post.puts "---"
    post.puts "{% include zhangxin/setup %}"
  end
end # task :post
...
namespace :theme do
  ...
  #        The theme must be installed into your zhangxin framework.
  #
  # Examples
  #
  ...
  task :switch do
    ...
    Dir.glob("#{theme_path}/*") do |filename|
      ...
      open(File.join(CONFIG['layouts'], File.basename(filename)), 'w') do |page|
        ...
        page.puts "{% include zhangxin/setup %}"
  ...
  task :install do
    ...
    packaged_theme_path = zhangxin::Path.build(:theme_packages, :node => name)
    ...
    packaged_theme_files.each do |filename|
      file_install_path = File.join(zhangxin::Path.base, filename)
  ...
  # The theme must be structured using valid zhangxin API.
  ...
  task :package do
    name = ENV["name"].to_s.downcase
    theme_path = zhangxin::Path.build(:themes, :node => name)
    asset_path = zhangxin::Path.build(:theme_assets, :node => name)
    ...
    packaged_theme_path = zhangxin::Path.build(:themes, :root => zhangxin::Path.build(:theme_packages, :node => name))
    mkdir_p packaged_theme_path
    cp_r theme_path, packaged_theme_path
    
    ## Mirror theme's asset directory
    packaged_theme_assets_path = zhangxin::Path.build(:theme_assets, :root => zhangxin::Path.build(:theme_packages, :node => name))
    ...
    open(zhangxin::Path.build(:theme_packages, :node => "#{name}/packager.yml"), "w") do |page|
      page.puts packager.to_yaml
    end
    
    puts "=> '#{name}' theme is packaged and available at: #{zhangxin::Path.build(:theme_packages, :node => name)}"
  end
...
def theme_from_git_url(url)
  tmp_path = zhangxin::Path.build(:theme_packages, :node => "_tmp")
  abort("rake aborted: system call to git clone failed") if !system("git clone #{url} #{tmp_path}")
  manifest = verify_manifest(tmp_path)
  new_path = zhangxin::Path.build(:theme_packages, :node => manifest["name"])
